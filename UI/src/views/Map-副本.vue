...
export default {
  ...
  methods: {
    ...
    async focusPoi(poi) {
      ...
      // 即使已经有amapDetails，我们也尝试保存到数据库
      else if (poi.amapDetails) {
        try {
          // 检查数据库中是否已存在此POI信息
          const checkResult = await axios.get(`/api/poiinfo/info/${poi.id}`);
          if (!checkResult.data) {
            // 如果数据库中不存在，则保存
            const saveResult = await this.savePoiToDatabase(poi, poi.amapDetails);
            if (saveResult) {
    ...
    async fetchPoiDetails(poi) {
      try {
        ...
        console.error('获取数据库POI信息出错:', error);
        if (error.response) {
          console.error('错误响应:', {
      ...
      try {
        ...
        // 使用统一的savePoiToDatabase方法保存到数据库
        const saveResult = await this.savePoiToDatabase(poi, poi.amapDetails);
        if (saveResult) {
          console.log('POI信息保存成功:', poi.name);
        } else {
          console.error('POI信息保存失败:', poi.name);
        }
    ...
    async savePoiToDatabase(poi, details) {
      try {
        ...
        if (typeof safePoiId !== 'string') {
          console.log(`POI ID类型不是字符串，当前类型: ${typeof safePoiId}，值: ${safePoiId}`);
        ...
        console.error(`POI ${poi.name} 的信息保存失败:`, error.message);
        // 添加更详细的错误信息记录
        if (error.response) {
          // 服务器响应了，但状态码不在2xx范围
          console.error('错误响应:', {
